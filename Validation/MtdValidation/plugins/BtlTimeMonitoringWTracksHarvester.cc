...
class BtlTimeMonitoringWTracksHarvester : public DQMEDHarvester {
public:
  explicit BtlTimeMonitoringWTracksHarvester(const edm::ParameterSet& iConfig);
  ~BtlTimeMonitoringWTracksHarvester() override;
...
std::pair<double, double> BtlTimeMonitoringWTracksHarvester::vector_Mean_MeanError(const std::vector<double>& data) const {
...
std::tuple<double, double, double> BtlTimeMonitoringWTracksHarvester::computeMean_MeanError(MonitorElement* me) const {
    if (!me)
        return {0., 0., 0.};
    ...
    const double threshold = 0.95 * totalCounts; //VALORE TRONCAMENTO CODA
...
BtlTimeMonitoringWTracksHarvester::BtlTimeMonitoringWTracksHarvester(const edm::ParameterSet& iConfig)
    : folder_(iConfig.getParameter<std::string>("folder")) {}

BtlTimeMonitoringWTracksHarvester::~BtlTimeMonitoringWTracksHarvester() {}

// ------------ endjob tasks ----------------------------
void BtlTimeMonitoringWTracksHarvester::dqmEndJob(DQMStore::IBooker& ibook, DQMStore::IGetter& igetter) {
...
//    edm::LogError("BtlTimeMonitoringWTracksHarvester") << "Monitoring histograms not found!" << std::endl;
//    return;
//  }
...
for (unsigned int ru = 1; ru <= nRU; ++ru) {
    ...
    out_RU_Slice_Tray << "WTRACKS: RU SLICE " << ru << "--------------------------------------------------- \n";

    for (unsigned int t = 1; t <= nTray; ++t) {
        std::string histoname = folder_ + "BtlHitTime_SingleRU_" + std::to_string(ru) + "_TR" + std::to_string(t);
        MonitorElement* me = igetter.get(histoname);

        if (!me) {
            edm::LogWarning("BtlTimeMonitoringWTracksHarvester") << "Histogram not found: " << histoname;
            continue;
        }
...
//-----------------------------------------
    ...
    out_RU_Slice_Tray << "-----------------------------------------------------------------------";
    edm::LogPrint("BtlTimeMonitoringWTracksHarvester") << out_RU_Slice_Tray.str();
}

std::ostringstream out_RU_Slice_Mean;
out_RU_Slice_Mean << "WTRACKS:--- MEAN VALUES RU ---\n";
...
out_RU_Slice_Mean << "-----------------------------------------------------------------------";    
edm::LogPrint("BtlTimeMonitoringWTracksHarvester") << out_RU_Slice_Mean.str();
...
for (unsigned int i = 1; i <= nSM; ++i) {
  std::string histoname = folder_ + "BtlHitTimePhiSlice_" + std::to_string(i);
  ...
  SMmeanErrorUncertainties.push_back(Err_meanError);
}

std::ostringstream out_SMslice;
out_SMslice << "WTRACKS:------ SM Slice Index ------\n";
...
for (unsigned int i = 0; i < nSM; ++i) {
    out_SMslice << std::setw(10) << i+1
		...
		<< std::setw(15) << SMmeanErrorUncertainties[i]
                << "\n";
}

out_SMslice << "-----------------------------------------------------------------------";
edm::LogPrint("BtlTimeMonitoringWTracksHarvester") << out_SMslice.str();
...
for (unsigned int ru = 1; ru <= nRU; ++ru) {
    ...
    out_Cal_SMinRU << "WTRACKS: --- RU INDEX " << ru << "------------------------------------------------ \n";

    for (unsigned int t = 1; t <= nSMinRU; ++t) {
        std::string histoname = folder_ + "BtlHitTimeSingleSmInRu_" + std::to_string(ru) + "_SM" + std::to_string(t);
        MonitorElement* me = igetter.get(histoname);
    ...
    out_Cal_SMinRU << "-----------------------------------------------------------------------";
    edm::LogPrint("BtlTimeMonitoringWTracksHarvester") << out_Cal_SMinRU.str();
}
...
out_CRYslice << "WTRACKS: --- CRYSTAL Slice Index ------\n";
out_CRYslice
...
out_CRYslice << "-----------------------------------------------------------------------";
edm::LogPrint("BtlTimeMonitoringWTracksHarvester") << out_CRYslice.str();
...
// TOTAL RU SLICE:
std::vector<double> RuSliceMean;
std::vector<double> RuSliceMeanErrors;


for (unsigned int i = 1; i <= nRU; ++i) {
  std::string histoname = folder_ + "TimeRUSlice_pTCut3_" + std::to_string(i);
  ...
  RuSliceMean.push_back(mean);
  RuSliceMeanErrors.push_back(meanError);

}

std::ostringstream DELTA_RUslice;
DELTA_RUslice << "WTRACKS: --- RU Slice DELTA ------\n";
DELTA_RUslice
    << std::setw(10) << "RU Index"
    << std::setw(15) << "Mean"
    << std::setw(15) << "Er Mean"
    << "\n";

    for (unsigned int i = 0; i < nRU; ++i) {
    DELTA_RUslice << std::setw(10) << i+1
                << std::setw(15) << RuSliceMean[i]
                << std::setw(15) << RuSliceMeanErrors[i]
                << "\n";
}
DELTA_RUslice << "-----------------------------------------------------------------------";
edm::LogPrint("BtlTimeMonitoringWTracksHarvester") << DELTA_RUslice.str();
...
void BtlTimeMonitoringWTracksHarvester::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;

  desc.add<std::string>("folder", "MTD/BTL/RecoTimingWTracks/");

  descriptions.add("btlTimeMonitoringWTracksPostProcessor", desc);
}



DEFINE_FWK_MODULE(BtlTimeMonitoringWTracksHarvester);
